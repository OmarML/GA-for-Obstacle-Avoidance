...
class Particle:
    def __init__(self, x, y, size, trajectory, R, G, B, ID):
        self.x = x
        self.y = y
        self.size = size
        self.colour = (R, G, B)
        ...
        if self.present:
            ...
            if len(self.paths) > 2:
                for i in range(len(self.paths)):
                    pygame.draw.circle(screen, self.colour, (int(self.paths[i][0]), int(self.paths[i][1])), 1, self.thickness)
...
for pedestrian in Pedestrian_IDs:
    ...
    my_particles.append(
        Particle(x1, y1, size, [x, y], random.randint(20, 255), random.randint(20, 255), random.randint(20, 255), pedestrian))
        ...
        if current <= max:
            ...
            if not particle.state():
                if not particle.considered:
                    pedestrian_count += 1
                    existing_particles.remove(particle)
